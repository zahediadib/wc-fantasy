...
function TeamPosterColumn({team}) {
    return (
        <div className="rounded-3xl border border-white/30 bg-white/10  p-6 shadow-[0_15px_60px_rgba(0,0,0,0.35)]">
            <div className="flex items-center gap-4">
                <img src={team.flagUrl} alt={team.countryName}
                     className="w-16 h-11 object-cover rounded-lg ring-1 ring-white/60" crossOrigin="anonymous"/>
                <div>
                    <div
                        className="text-3xl font-black text-white drop-shadow-[0_4px_16px_rgba(0,0,0,0.85)]">{team.countryName}</div>
                    <div className="text-sm text-yellow-200 font-bold">{team.tierName}</div>
                </div>
                <div
                    className="mr-auto text-5xl font-black text-cyan-200 drop-shadow-[0_4px_18px_rgba(0,255,255,0.35)]">{toFaDigits(team.matchResult ?? 0)}</div>
            </div>

            <div className="mt-5 min-h-[300px] rounded-2xl border border-white/25 bg-black/30 p-4">
                <div className="text-[15px] text-[center] font-bold text-white mb-6">جزئیات امتیازات فانتزی</div>
                <div className="space-y-1.5">
                    {team.fantasyStats.map((s, idx) => (
                        <div key={`${team.countryName}-${idx}`}
                             className="border-b border-white/30 flex items-center justify-between text-[15px] text-white/95">
                            <span>{s.title}</span>
                            <span style={{direction: "ltr"}}
                                  className={`mono font-bold ${s.finalScore >= 0 ? "text-emerald-300" : "text-rose-300"}`}>{fmtSigned(s.finalScore)}</span>
                        </div>
                    ))}
                </div>
            </div>

            <div className="mt-4 grid grid-cols gap-3">
                <div className="rounded-xl border border-yellow-300/40 bg-yellow-500/10 p-3">
                    <div className="text-[13px] text-yellow-100">جمع امتیاز</div>
                    <div style={{direction: "ltr"}}
                         className={`mono mt-1 text-2xl font-black ${team.totalMatchScore >= 0 ? "text-emerald-300" : "text-rose-300"}`}>{fmtSigned(team.totalMatchScore)}</div>
                </div>
                {team.usersROI?.length > 0 && (
                    <div className="rounded-xl border border-cyan-300/40 bg-cyan-500/10 p-3">
                        <div className="text-[13px] text-cyan-100">بازگشت سرمایه تیم (بعد از این بازی)</div>
                        <div className="text-2xl text-cyan-50 mt-1 space-y-1 max-h-24 overflow-hidden">
                            {team.usersROI.slice(0, 3).map((u) => (
                                <div key={`${team.countryName}-${u.userId}`}
                                     className="flex items-center justify-between gap-2">
                                    <span className="font-black truncate"></span>
                                    <span style={{direction: "ltr"}}
                                          className={`font-black mono ${u.roi >= 0 ? "text-emerald-300" : "text-rose-300"}`}>{fmtSigned(u.roi)}</span>
                                </div>
                            ))}
                        </div>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
                <Button size="sm" variant="outline"
                        className="border-cyan-300/40 text-cyan-200 hover:bg-cyan-500/10 text-sm">
                    <ImageIcon className="w-4 h-4"/> خروجی پوستر
                </Button>
            </DialogTrigger>
            <DialogContent className="bg-[#0F1626] text-white border-white/10 max-w-5xl" dir="rtl">
                <DialogHeader>
                    <DialogTitle className="text-xl">پوستر بازی تسویه‌شده</DialogTitle>
                    <DialogDescription className="text-base">تم جام جهانی ۲۰۲۶ با خروجی PNG با کیفیت
                        بالا</DialogDescription>
                </DialogHeader>

                {loading ? (
                    <div className="py-16 flex items-center justify-center text-gray-300 gap-2 text-base"><Loader2
                        className="w-5 h-5 animate-spin"/> در حال آماده‌سازی پوستر...</div>
                ) : !matchData ? (
                    <div className="py-16 text-center text-rose-300 text-lg">داده پوستر قابل نمایش نیست.</div>
                ) : (
                    <>
                        ...
                        <div className="flex justify-end gap-2">
                            <Button onClick={exportPoster} disabled={exporting}
                                    className="bg-gradient-to-r from-cyan-500 to-fuchsia-500 text-white font-bold text-base px-6">
                                {exporting ? <Loader2 className="w-5 h-5 animate-spin"/> :
                                    <Download className="w-5 h-5"/>} دانلود PNG
                            </Button>
                        </div>
                    </>
                )}
    ...
    return (
        ...
        >
            ...
            <div className="relative h-full p-10 flex flex-col">
                <div
                    className="rounded-3xl border border-yellow-200/40 bg-white/10 p-6 text-center shadow-[0_20px_80px_rgba(0,0,0,0.35)]">
                    <div className="text-sm tracking-[0.2em] text-yellow-100/90">WORLD CUP 2026 · FANTASY SETTLEMENT
                    </div>
                    <div className="mt-6 mb-6 text-5xl font-black drop-shadow-[0_4px_18px_rgba(0,0,0,0.7)]">نتیجه بازی
                    </div>
                    <div className="mt-2 flex items-center justify-center gap-4 text-base text-cyan-100">
